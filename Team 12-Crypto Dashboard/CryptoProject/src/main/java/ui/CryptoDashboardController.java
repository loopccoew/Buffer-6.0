...
public class CryptoDashboardController {
    ...
    // Add missing method for accessing price
    public double getPrice() {
        return price.get();
    }

    // Add missing high/low getters
    ...
    // Add missing percent change getter
    ...
    private void updateChart(CryptoData crypto) {
        if (crypto == null)
            return;
        LineChart<Number, Number> chart = GraphUtil.createMiniGraph(crypto.getPrice());
        // ...rest of existing code...
    }

    private void showTrending() {
        // Replace constructor calls with proper parameters
        CryptoData btc = new CryptoData("BTC", "Bitcoin");
        CryptoData eth = new CryptoData("ETH", "Ethereum");
        CryptoData sol = new CryptoData("SOL", "Solana");
        // ...rest of the method...
    }

    private void translateTableContent() {
        final String targetLang = getLanguageCode(displayLanguage.getValue());
        if (!targetLang.equals(currentLanguage)) {
            currentLanguage = targetLang;

            Platform.runLater(() -> {
                // Now targetLang is effectively final
                cryptoTable.getColumns().forEach(column -> TranslationService.translate(column.getText(), targetLang)
                        .thenAccept(column::setText));

                getAllNodes(root).forEach(node -> {
                    if (node instanceof Labeled) {
                        Labeled labeled = (Labeled) node;
                        TranslationService.translate(labeled.getText(), targetLang)
                                .thenAccept(labeled::setText);
                    }
                });
    ...
    private void updatePriceChart(CryptoData crypto) {
        ...
        sevenDayChart.getData().clear();
        final XYChart.Series<String, Number> series = new XYChart.Series<>();
        final List<Double> prices = crypto.getSevenDayPrices();
        final List<String> dates = getLast7Days();

        // Now using final variables in lambda
        for (int i = 0; i < prices.size() && i < dates.size(); i++) {
            final XYChart.Data<String, Number> data = new XYChart.Data<>(dates.get(i), prices.get(i));
            series.getData().add(data);

            final Node node = data.getNode();
            if (node != null) {
                node.setOnMouseEntered(e -> showTooltip(node, String.format("$%.2f", data.getYValue())));
            }
        }
