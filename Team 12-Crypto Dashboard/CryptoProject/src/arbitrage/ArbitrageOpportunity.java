...
public class ArbitrageOpportunity {
    ...
    // Add getters for table display
    public String getMarket() {
        return buyExchange.get() + " → " + sellExchange.get();
    }
    ...
    public double getProfitUSD() {
        return sellPrice.get() - buyPrice.get();
    }

    @Override
    public String toString() {
        return String.format("%s: Buy at %s ($%.2f), Sell at %s ($%.2f), Profit: %.2f%%",
                coin.get(), buyExchange.get(), buyPrice.get(), sellExchange.get(), sellPrice.get(),
                profitPercentage.get());
    }
}
